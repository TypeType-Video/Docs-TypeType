...
3. From a report, an admin can open a **prefilled GitHub issue**: the app builds a
   ...
   (the report and its context), the `bug` label, and, when `GITHUB_ISSUE_TEMPLATE`
   is set, that template. **Domains are redacted** from the body first, so a report
   never leaks where your instance is hosted.
...
::: warning Template vs prefilled body
GitHub does not reliably honour a prefilled `body` and a `template` in the same link,
the two can override each other, and on mobile the prefilled fields may be dropped
when a template exists. If you want the full prefilled report to show, leave
...
- **Type**, what kind of report it is: `bug`, `feature request`, `enhancement`,
  `documentation`, or `question`. The in-app forms set this for you automatically.
